...
export async function POST(req: Request) {
  ...
  // Baseline: last 14 days (latest snapshot per day)
  ...
  const todaysLatest = snapshotsToday.length
    ? snapshotsToday[snapshotsToday.length - 1]
    : null;
  ...
  const risk =
    (typeof ai.risk === "string" &&
    ["OK", "LOW", "MED", "HIGH"].includes(ai.risk)
      ? ai.risk
